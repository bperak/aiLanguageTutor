...
export const GrammarPatternCard: React.FC<GrammarPatternCardProps> = ({
  ...
  const getLevelColor = (textbook: string) => {
    const colorMap: Record<string, string> = {
      "入門(りかい)": "bg-green-500/10 text-green-600",
      "初級1(りかい)": "bg-blue-500/10 text-blue-600",
      "初級2(りかい)": "bg-blue-200 text-blue-700",
      初中級: "bg-yellow-500/10 text-yellow-600",
      中級1: "bg-orange-100 text-orange-800",
      中級2: "bg-red-500/10 text-red-600",
    };
    return colorMap[textbook] || "bg-muted text-foreground";
  };
  ...
  const getNextReviewStatus = (nextReviewDate?: string) => {
    if (!nextReviewDate) return { text: "Not studied", color: "text-muted-foreground" };
    const now = new Date();
    ...
    if (diffDays === 0)
return { text: "Review today", color: "text-blue-600" };
    if (diffDays === 1)
return { text: "Review tomorrow", color: "text-yellow-600" };
  ...
  return (
    <Card
      className={`grammar-pattern-card hover:shadow-lg transition-all duration-300 ${className} ${isFlipped ? "transform" : ""}`}
    >
      {" "}
      <CardHeader className="pb-3">
          ...
          <div className="mb-3">
            ...
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="flex items-center gap-2">
                {" "}
                <Star
                  className={`w-4 h-4 ${getMasteryColor(srsData.mastery_level)}`}
                />{" "}
                <span className="text-sm font-medium">
                  Mastery Level {srsData.mastery_level}/5
                </span>{" "}
              </div>{" "}
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex-1">
            ...
            <p
              className={`text-muted-foreground italic transition-opacity duration-300 ${isFlipped ? "opacity-30" : "opacity-100"}`}
            >
              {" "}
              {isFlipped
                ? "[Hidden - Click eye to reveal]"
                : pattern.pattern_romaji}{" "}
            </p>{" "}
          </div>{" "}
      ...
      <CardContent>
        ...
        <div className="space-y-4">
          {" "}
          {/* Description (AI what_is) - optional
if available; shown above Example */}{" "}
          {pattern.what_is && (
          ...
          <div className="example-section">
            ...
            <div className="bg-muted p-3 rounded-md">
              {" "}
              <p
                className={`text-lg font-medium text-foreground mb-1 transition-opacity duration-300 ${isFlipped ? "opacity-30" : "opacity-100"}`}
              >
                {" "}
                {isFlipped
                  ? "[Hidden - Click eye to reveal]"
              ...
              <p
                className={`text-sm text-muted-foreground italic transition-opacity duration-300 ${isFlipped ? "opacity-30" : "opacity-100"}`}
              >
                {" "}
            ...
            {interactive && onMarkAsStudied && (
              <div className="srs-section">
                ...
                {!showSRSButtons ? (
                  ...
                  >
                    {" "}
                    <CheckCircle className="w-4 h-4 mr-2" /> Quick Study{" "}
                  </Button>
                ) : (
