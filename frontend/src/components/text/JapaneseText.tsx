  ...
  return (
    <Container className={`${spacing} ${className}`}>
      ...
      {settings.showTranslation && translationText && (
        <div
          className={`text-sm text-foreground border-l-2 border-blue-300 pl-3 ${
            inline ? "inline ml-2" : ""
          }`}
        >
          {translationText}
...
export function InlineJapanese({
  data,
  className,
}: Omit<JapaneseTextProps, "inline">) {
