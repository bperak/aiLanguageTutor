  ...
  const testConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setResult(
          `✅ Connection successful! Response: ${JSON.stringify(data, null, 2)}`
        );
      } else setResult(`❌ Connection failed with status: ${response.status}`);
    } catch (error: any) {
  ...
  const testLexicalEndpoint = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setResult(
          `✅ Lexical endpoint successful! Nodes: ${
            data.nodes?.length || 0
        ...
        setResult(
          `❌ Lexical endpoint failed with status: ${response.status} - ${errorText}`,
        );
      }
    } catch (error: any) {
    ...
    <div className="p-4 border border-border rounded-lg bg-muted">
      ...
      <div className="space-x-2 mb-4">
        <button
          ...
          disabled={loading}
          className="px-4 py-2 bg-blue-500/100 text-white rounded hover:bg-blue-600 disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 bg-green-500/100 text-white rounded hover:bg-green-600 disabled:opacity-50"
        >
