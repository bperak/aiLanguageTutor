...
import type { ExercisesCard as ExercisesCardType, Exercise } from "@/types/lesson-root"

interface ExercisesCardProps {
  data: ExercisesCardType
}

function MatchExercise({ exercise }: { exercise: Extract<Exercise, { exercise_type: "match" }> }) {
...
function FillBlankExercise({ exercise }: { exercise: Extract<Exercise, { exercise_type: "fill_blank" }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showFeedback && (
        <div className={`p-3 rounded ${isCorrect ? "bg-green-100 dark:bg-green-900" : "bg-red-100 dark:bg-red-900"}`}>
          {isCorrect ? "Correct!" : `Possible answers: ${exercise.answer_key_en.join(", ")}`}
        </div>
      )}
    </div>
  )
...
function OrderExercise({ exercise }: { exercise: Extract<Exercise, { exercise_type: "order" }> }) {
  const [currentOrder, setCurrentOrder] = useState<number[]>([...Array(exercise.segments_ja.length).keys()])
  const [showFeedback, setShowFeedback] = useState(false)
        ...
        {currentOrder.map((segIdx, displayIdx) => (
          <div key={displayIdx} className="flex items-center gap-2">
            <div className="flex flex-col gap-1">
              <Button size="sm" variant="outline" onClick={() => moveUp(displayIdx)} disabled={displayIdx === 0}>
                ↑
              </Button>
              <Button size="sm" variant="outline" onClick={() => moveDown(displayIdx)} disabled={displayIdx === currentOrder.length - 1}>
                ↓
              </Button>
            </div>
            <div className="flex-1 p-3 border rounded">{exercise.segments_ja[segIdx]}</div>
          </div>
        ))}
      </div>
      ...
      <CardContent className="space-y-6">
        {data.items.map((exercise, idx) => (
          <Card key={exercise.id} className="p-4">
            ...
            {exercise.exercise_type === "match" && <MatchExercise exercise={exercise} />}
            {exercise.exercise_type === "fill_blank" && <FillBlankExercise exercise={exercise} />}
            {exercise.exercise_type === "order" && <OrderExercise exercise={exercise} />}
            </CardContent>
          </Card>
        ))}
      </CardContent>
