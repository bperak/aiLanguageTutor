...
export function LessonDialogueCard({
  data,
  canDoId,
}: LessonDialogueCardProps) {
    ...
    () =>
      turns.map((t) => ({
        ...
        japanese: {
          ...
          romaji: t.ja.romaji || "",
          furigana: Array.isArray(t.ja.furigana)
            ? t.ja.furigana
            : t.ja.std
            ? [{ text: t.ja.std }]
            : [],
          translation:
            typeof t.ja.translation === "string"
              ? t.ja.translation
              : t.ja.translation?.en || "",
        },
      })),
  ...
  const onMore = async () => {
    ...
    try {
      ...
      // Backend returns full_turns = existing + new, so we use all returned turns
      const allTurns = (card.dialogue_turns || []).map((t: any) => ({
        speaker: t.speaker,
        ja: {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        {data.notes_en && (
          <p className="text-sm text-muted-foreground mt-2">{data.notes_en}</p>
        )}
      </CardHeader>
      <CardContent className="space-y-4">
              ...
              <Image
                ...
                onError={(e) => {
                  if (process.env.NODE_ENV === "development") {
                    console.error(
                      "[LessonDialogueCard] Image load error:",
                      imageUrl,
                      e
                    );
                  }
        ...
        <div className="space-y-3">
          {turns.map((turn, idx) => (
            ...
            >
              <div
                className={`max-w-[80%] ${idx % 2 === 0 ? "" : "text-right"}`}
              >
                ...
                <div
                  className={`rounded-lg p-3 ${
                    idx % 2 === 0 ? "bg-muted" : "bg-blue-500/10"
                  }`}
                >
