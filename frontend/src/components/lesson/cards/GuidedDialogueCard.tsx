...
import { sendGuidedTurn, flushGuidedState, getGuidedInitialMessage } from "@/lib/api/guided-dialogue"
import type { GuidedDialogueCard as GuidedDialogueCardType, GuidedTurnResponse } from "@/types/lesson-root"
...
export function GuidedDialogueCard({ data, sessionId, initialStageIdx = 0 }: GuidedDialogueCardProps) {
  ...
  const [collapsedFeedback, setCollapsedFeedback] = useState<Set<number>>(new Set())
  ...
  useEffect(() => {
    const fetchInitialMessage = async () => {
      ...
      try {
        ...
        console.log("Fetching initial message for session:", sessionId, "stage:", currentStageIdx)
        ...
        if (initialMessage.status === "ok" || initialMessage.message) {
          const aiMessage: Message = {
            ...
            transliteration: initialMessage.transliteration || null,
            translation: initialMessage.translation || null,
        ...
        const defaultMessage: Message = {
          ...
          teachingDirection: "Let's start practicing. Use the hints above to guide your response.",
        }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiMessage: Message = {
        ...
        transliteration: response.transliteration || null,
        translation: response.translation || null,
        teachingDirection: response.feedback.teaching_direction || null,
      ...
      if (response.stage_progress.advanced) {
        ...
        if (!response.stage_progress.completed) {
          try {
            ...
            console.error("Failed to fetch initial message for new stage:", error)
          }
        } else {
          setMessages((prev) => [
            ...prev,
            {
              role: "ai",
              content: "🎉 Congratulations! You've completed all guided dialogue stages!",
            },
          ])
        }
      }
  ...
  if (isCompleted) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-2xl">Guided Dialogue - Completed! 🎉</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <p className="text-lg mb-4">You've successfully completed all stages!</p>
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-sm mb-2">
            <span>Stage {currentStageIdx + 1} of {data.stages.length}</span>
      ...
      <CardContent className="space-y-4">
        {/* Hints toggle */}
        {currentStage && currentStage.hints && currentStage.hints.length > 0 && (
          <div>
            <Button
              variant="outline"
              size="sm"
            ...
            {showHints && (
              <div className="mt-2 space-y-2">
                {(() => {
                  // Filter out empty hints and log for debugging
                  const validHints = currentStage.hints.filter((hint, idx) => {
                    // Check for JPText format (std, kanji, or translation)
                    const hasJPTextFormat = hint && (
                      hint.std || 
                      hint.kanji || 
                      (hint.translation && (typeof hint.translation === 'string' || hint.translation.en || hint.translation.ja))
                    )
                    
                    // Check for bilingual format (en/ja directly on hint object)
                    const hasBilingualFormat = hint && (hint.en || hint.ja)
                    
                    const hasContent = hasJPTextFormat || hasBilingualFormat
                    
                    if (!hasContent && process.env.NODE_ENV === 'development') {
                      console.warn("Empty hint detected at index", idx, "hint data:", hint)
                    }
                    return hasContent
                  })
                  ...
                  if (validHints.length === 0) {
                    return (
                      <div className="p-3 bg-yellow-100 dark:bg-yellow-900 rounded border-l-4 border-yellow-500 text-sm text-yellow-800 dark:text-yellow-200">
                        No hints available for this stage. The hints data may be empty or malformed.
                      </div>
                    )
                  }
                  
                  return validHints.map((hint, idx) => {
                    // Convert bilingual format to JPText format for JapaneseText component
                    let hintData = hint
                    if (hint.en || hint.ja) {
                      // Convert {en: '...', ja: '...'} to JPText format
                      hintData = {
                        std: hint.ja || '',
                        translation: { en: hint.en || '' }
                      }
                    }
                    
                    return (
                      <div key={idx} className="p-3 bg-yellow-50 dark:bg-yellow-950 rounded border-l-4 border-yellow-400">
          ...
          <div className="space-y-6">
            {messages.map((msg, idx) => (
              <div key={idx} className="space-y-2">
                {/* Conversation Bubble */}
                <div className={`flex ${msg.role === "learner" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] ${msg.role === "learner" ? "text-right" : ""}`}>
                    ...
                    {msg.role === "ai" && (msg.transliteration || msg.translation) && (
                      <div className="mt-3 space-y-2 pt-3 border-t border-gray-100">
                        {msg.transliteration && (
                          <div className="bg-blue-50 dark:bg-blue-950 p-2 rounded border-l-4 border-blue-300">
                            <p className="text-xs font-semibold text-blue-800 dark:text-blue-200 mb-1">🔤 Pronunciation (Romaji):</p>
                            <p className="text-sm text-blue-700 dark:text-blue-300 font-mono italic">{msg.transliteration}</p>
                          </div>
                        )}
                        {msg.translation && (
                          <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded border-l-4 border-gray-300">
                            <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 mb-1">🌍 English Translation:</p>
                            <p className="text-sm text-gray-700 dark:text-gray-300">{msg.translation}</p>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950 dark:to-orange-950 border border-amber-200 dark:border-amber-800 rounded-lg overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-amber-800 dark:text-amber-200 font-medium text-sm">🧑‍🏫 Teaching Direction & Strategy</span>
                        </div>
                      ...
                      {!collapsedFeedback.has(idx) && (
                        <div className="px-3 pb-3 space-y-3">
                          <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border-l-4 border-amber-400">
                            <h5 className="font-medium text-amber-800 dark:text-amber-200 mb-1 text-sm">Teaching Strategy:</h5>
                            <p className="text-amber-700 dark:text-amber-300 text-sm">{msg.teachingDirection}</p>
                          </div>
                          
                          {msg.hints && msg.hints.length > 0 && (
                            <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg border-l-4 border-blue-400">
                              <h5 className="font-medium text-blue-800 dark:text-blue-200 mb-2 flex items-center gap-1 text-sm">
                                💡 Learning Tips
                              </h5>
                ...
                {msg.role === "learner" && msg.feedback && (
                  <div className="ml-4 mt-2 flex flex-wrap gap-2 justify-end">
                    <Badge variant={msg.feedback.pattern_matched ? "default" : "destructive"}>
                      Pattern: {msg.feedback.pattern_matched ? "✓" : "✗"}
                    </Badge>
                    <Badge variant={msg.feedback.word_count_ok ? "default" : "secondary"}>
