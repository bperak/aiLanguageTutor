...
function WordCard({ word }: { word: WordRef }) {
  ...
  const href = kanji ? `/lexical/graph?center=${encodeURIComponent(kanji)}` : undefined
  ...
  return (
    ...
    >
      <div className="flex items-baseline gap-2 mb-1 flex-wrap">
        <div className="text-base md:text-lg font-medium text-gray-900 break-words">{kanji}</div>
        {pos && (
          <span className="text-xs text-emerald-600 font-mono uppercase shrink-0">{pos}</span>
        )}
      </div>
      
      {reading && (
        <div className="text-xs md:text-sm text-gray-600 mb-1 break-words">{reading}</div>
      )}
      
      {romaji && (
        <div className="text-xs text-gray-500 italic mb-1 break-words">{romaji}</div>
      )}
...
function GrammarCard({ grammar }: { grammar: GrammarRef }) {
  ...
  const href = patternId ? `/grammar/study/${encodeURIComponent(patternId)}` : undefined
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-2 mb-1 flex-wrap">
        <div className="text-sm md:text-base font-medium text-gray-900 break-words flex-1">{pattern}</div>
        {classification && (
      ...
      {!href && (
        <div className="text-xs text-gray-500 italic">Pattern not mapped</div>
      )}
    </a>
  )
}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {grammar.slice(0, 15).map((g, i) => (
              <GrammarCard key={`grammar-${i}-${g.id || g.pattern}`} grammar={g} />
            ))}
          </div>
          {grammar.length > 15 && (
            <div className="mt-2 text-xs text-gray-500 text-center">
