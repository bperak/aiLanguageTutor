...
function isJapaneseTextObject(item: any): boolean {
  return typeof item === 'object' && item !== null && 
         (item.kanji || item.romaji || item.translation || item.furigana)
}
  ...
  return (
    <Card className="border-0 shadow-sm bg-white/70 backdrop-blur">
      <CardHeader>
        ...
        {card.subtitle ? <CardDescription>{card.subtitle}</CardDescription> : null}
      </CardHeader>
      <CardContent className="space-y-3">
          ...
          <>
            ...
            {card.body?.meta ? (
              isJapaneseTextObject(card.body.meta) ? (
                ...
                <MetaMarkdown text={card.body.meta} showTranslation={showTranslations} />
              )
            ) : null}
          </>
        )}
        ...
        {Array.isArray(card.media) && card.media.length > 0 ? (
          <div className="grid grid-cols-2 gap-3">
            {card.media.map((m, i) => (
              <img key={i} src={m.path} alt={m.alt || "image"} className="w-full h-auto rounded" />
            ))}
          </div>
        ) : null}
      </CardContent>
...
export default function LessonRenderer({ 
  ...
  const showRomaji = level <= 2
  const showTranslations = level <= 3
  
  const header = variant.ui.header || {}
  const sections = variant.ui.sections || []
  const enhancedMap: Record<string, boolean> = (variant as any)?.metadata?.stage2EnhancedMap || {}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
        ...
        {level <= 2 && <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded text-xs">Beginner (with romaji)</span>}
        {level >= 3 && level <= 4 && <span className="px-2 py-0.5 bg-green-100 text-green-800 rounded text-xs">Intermediate</span>}
        {level >= 5 && <span className="px-2 py-0.5 bg-purple-100 text-purple-800 rounded text-xs">Advanced (no aids)</span>}
      </div>
      {(header.title || header.subtitle) && (
        <div className="rounded-xl border bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm">
          {header.title ? <div className="text-xl font-semibold">{header.title}</div> : null}
          {header.subtitle ? (
          ...
          {Array.isArray(header.chips) && header.chips.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {header.chips.map((c, i) => (
                <span key={i} className="text-xs rounded-full bg-slate-100 px-2 py-1 text-slate-700">
                  {c}
                </span>
              ))}
            </div>
          ...
          {sec.title ? (
            <div className="flex items-center gap-2 text-sm font-medium text-slate-700">
              ...
              {enhancedMap[sec.id] || enhancedMap[sec.type] ? (
                <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded text-[10px] uppercase tracking-wide">Enhanced</span>
              ) : null}
            </div>
          ) : null}
