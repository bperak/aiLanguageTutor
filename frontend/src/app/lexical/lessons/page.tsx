...
export default function LexicalLessonsPage() {
  ...
  useEffect(() => {
    ...
    async function loadSeeds() {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            json?.detail || `Failed to seed (status ${res.status})`
          );
        }
        if (isMounted) {
          setSeeds(json.items || []);
  ...
  async function generate() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          json?.detail || `Failed to generate (status ${res.status})`
        );
      }
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <select
          className="border border-input bg-background text-foreground px-2 py-1 rounded min-w-40"
          value={selected}
          onChange={(e) => setSelected(e.target.value)}
        >
          {seeds.map((s) => (
            <option key={s.kanji} value={s.kanji}>
              {s.kanji} {s.hiragana ? `(${s.hiragana})` : ""} —{" "}
              {s.translation || ""}
            </option>
          ))}
        </select>
        <button
          className="ml-auto border border-input px-3 py-1 rounded hover:bg-accent"
          onClick={generate}
      ...
      {content && (
        <div className="rounded border p-3 space-y-2">
          <div className="text-sm text-muted-foreground">Generated Lesson</div>
          ...
          {readability && (
            <div className="text-xs mt-2">
              ...
              {readability.available
                ? `${readability.level} (${readability.score?.toFixed?.(3)})`
                : "N/A"}
            </div>
          )}
        </div>
