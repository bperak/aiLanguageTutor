...
import { getPatternsDueForReview, getUserProgressSummary, recordStudyWithSRS, type PatternProgress } from '@/lib/api/grammar-progress';
...
export default function SRSPage() {
  ...
  useEffect(() => {
    if (dueItems.length > 0) {
      setSessionStats({ reviewed: currentReviewIndex, remaining: dueItems.length - currentReviewIndex });
    }
  }, [currentReviewIndex, dueItems.length]);
  ...
  const loadSRSData = async () => {
    try {
      ...
      // The API currently returns extended fields beyond PatternProgress.
      // Cast to DueReviewItem[] to enrich with overdue metadata without TS friction.
      const enhancedDueData = (dueData as unknown as DueReviewItem[]).map((item) => {
      ...
      // Map backend summary to local view model, filling missing fields.
      const mappedSummary: ProgressSummary = {
        total_patterns_studied: (summaryData as any).total_patterns_studied ?? 0,
        ...
        estimated_daily_reviews: (summaryData as any).estimated_daily_reviews ?? ((summaryData as any).patterns_due_today ?? 0),
      };
  ...
  const handleSRSGrade = async (grade: 'again' | 'hard' | 'good' | 'easy') => {
    ...
    try {
      await recordStudyWithSRS(currentItem.pattern_id, grade, 60, getConfidenceFromGrade(grade));
      ...
      if (currentReviewIndex < dueItems.length - 1) {
        ...
        await loadSRSData(); // Refresh data
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Loading your SRS schedule...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <Button variant="outline" onClick={() => setSessionActive(false)}>End Session</Button>
            </div>
          </div>
          <Progress value={((currentReviewIndex) / dueItems.length) * 100} className="w-full" />
        </div>
        ...
        <Card className="mb-6">
          <CardHeader className="pb-4">
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="text-3xl mb-2">{currentItem.pattern}</CardTitle>
                <p className="text-xl text-muted-foreground italic">{currentItem.pattern_romaji}</p>
                <div className="flex items-center gap-2 mt-3">
                  <Badge>{currentItem.textbook}</Badge>
                  <Badge variant="outline">{currentItem.classification}</Badge>
                  <div className="flex items-center gap-1">
                    <Star className={`w-4 h-4 ${getMasteryColor(currentItem.mastery_level)}`} />
                    <span className="text-sm font-medium">Level {currentItem.mastery_level}/5</span>
                  </div>
                </div>
              </div>
          ...
          <CardContent>
            {!showAnswer ? (
              <div className="text-center py-8">
                <p className="text-lg mb-6">How well do you remember this pattern?</p>
              ...
              <div className="space-y-6">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-lg font-medium mb-2">{currentItem.example_sentence}</p>
                  <p className="text-muted-foreground italic">{currentItem.example_romaji}</p>
                </div>

                <div className="text-center">
                  <p className="text-sm font-medium mb-4">How difficult was this?</p>
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      onClick={() => handleSRSGrade('again')}
                      variant="outline"
                      className="text-red-600 hover:bg-red-50 h-auto p-4 flex-col"
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="text-blue-600 hover:bg-blue-50 h-auto p-4 flex-col"
                    >
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="text-green-600 hover:bg-green-50 h-auto p-4 flex-col"
                    >
      ...
      {progressSummary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {dueItems.filter(item => item.days_overdue && item.days_overdue > 0).length} overdue
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{progressSummary.average_mastery_level.toFixed(1)}/5</div>
              <Progress value={(progressSummary.average_mastery_level / 5) * 100} className="mt-2" />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">{progressSummary.current_streak_days}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{progressSummary.total_study_sessions}</div>
          ...
          {dueItems.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                No patterns are due for review right now. Great job staying on top of your studies!
              </p>
              <Button variant="outline" onClick={() => window.location.href = '/grammar'}>
            ...
            <div className="space-y-4">
              {dueItems.slice(0, 10).map((item, index) => (
                <div key={item.pattern_id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-lg font-semibold">{item.pattern}</h3>
                        <span className="text-muted-foreground italic">{item.pattern_romaji}</span>
                        {getUrgencyBadge(item)}
                      </div>
                      <p className="text-gray-600 mb-2">{item.example_sentence}</p>
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <div className="flex items-center gap-1">
                          <Star className={`w-4 h-4 ${getMasteryColor(item.mastery_level)}`} />
                    ...
                    <Button 
                      variant="outline" 
                      size="sm"
                      onClick={() => window.location.href = `/grammar/study/${item.pattern_id}`}
                    >
