...
export default function CreateCanDoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!descriptionEn.trim() && !descriptionJa.trim()) {
      setError(
        "Please provide at least one description (English or Japanese)"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success && result.canDo) {
        showToast(
          "CanDo created successfully! Processing embeddings and relationships..."
        );
      ...
      <Card className="p-6">
        ...
        <CardContent>
          <p className="text-muted-foreground mb-6">
            Create a new CanDo descriptor. Provide at least one description
            (English or Japanese). The system will automatically:
          </p>
          <ul className="list-disc list-inside text-sm text-muted-foreground mb-6 space-y-1">
            <li>
              Translate the description if only one language is provided
            </li>
            <li>Generate titles in both languages</li>
            <li>
              Infer level, topic, skill domain, and type from the description
            </li>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <textarea
                ...
                className="w-full border border-input bg-background text-foreground rounded-md px-3 py-2 min-h-[120px] focus:outline-none focus:ring-2 focus:ring-ring"
                disabled={isCreating}
              />
            </div>
            ...
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 text-red-700 px-4 py-3 rounded">
                {error}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <Button
                ...
                disabled={
                  isCreating ||
                  (!descriptionEn.trim() && !descriptionJa.trim())
                }
                className="flex-1"
              >
          ...
          {isCreating && (
            <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded">
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Creating CanDo and processing... This may take a moment as the
                system:
              </p>
