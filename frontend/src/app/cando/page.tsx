  ...
  useEffect(() => {
    const fetchList = async () => {
      ...
      try {
        ...
        const data = await apiGet<{ items: CanDoItem[] }>(
          `/api/v1/cando/list?${params.toString()}`
        );
        ...
        const count = await apiGet<{ total: number }>(
          `/api/v1/cando/count?${params2.toString()}`
        );
    ...
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-2">
        <input
          ...
          placeholder="Search CanDo or Topic"
        ...
        <button
          ...
          className="border border-input rounded px-3 py-2 bg-primary text-primary-foreground hover:bg-primary/90"
        >
          Search
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-4 text-sm text-muted-foreground">
        <div>
          {loading
            ...
            : error
            ? error
            : `Showing ${items.length} of ${total} result(s)`}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="border border-input rounded px-2 py-1 hover:bg-accent"
          >
            Prev
          </button>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map((it) => {
          ...
          const hasTitleJa =
            it.titleJa &&
            typeof it.titleJa === "string" &&
            it.titleJa.trim().length > 0;
          const hasActualTitle = hasTitleEn || hasTitleJa;

          // Topic/category info (always available)
          const topicJa =
            it.primaryTopic && typeof it.primaryTopic === "string"
              ? it.primaryTopic.trim()
              : null;
          const topicEn =
            it.primaryTopicEn && typeof it.primaryTopicEn === "string"
              ? it.primaryTopicEn.trim()
              : null;

          // Use actual titles if available, otherwise use topic as title
          const displayTitleEn = hasTitleEn && it.titleEn
            ? it.titleEn.trim()
            : topicEn || it.uid;
          const displayTitleJa = hasTitleJa && it.titleJa
            ? it.titleJa.trim()
            : topicJa || null;
          ...
          return (
            ...
            >
              ...
              <CardContent className="flex-grow flex flex-col pt-0">
                  ...
                  <div className="mb-1.5">
                    <div className="flex flex-wrap gap-1">
                      ...
                      {topicEn && topicEn !== topicJa && (
                        <span className="text-xs bg-green-500/10 text-green-600 px-2 py-0.5 rounded border border-green-500/30">
                          {topicEn}
                        </span>
                      )}
                    </div>
                ...
                {it.skillDomain && (
                  <div>
                    <span className="text-xs bg-purple-500/10 text-purple-700 px-2 py-0.5 rounded">
                      {it.skillDomain}
                    </span>
                  </div>
                )}
              ...
              <CardFooter className="pt-2 border-t flex gap-2">
                <a
                  ...
                  className="flex-1 text-center px-3 py-1.5 bg-primary text-primary-foreground rounded-md text-xs md:text-sm font-medium hover:bg-primary/90 transition-colors active:bg-primary"
                >
                  Start Lesson
                </a>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="px-3 py-1.5 border border-input rounded-md text-xs md:text-sm text-muted-foreground hover:bg-accent transition-colors active:bg-accent"
                >
                  Data
                </a>
              </CardFooter>
