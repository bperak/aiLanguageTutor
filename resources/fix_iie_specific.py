#!/usr/bin/env python3
"""
Fix the specific "いいえ" case: "I ie" -> "Iie"
"""
...
def fix_iie_specific(driver):
    """Fix the specific 'いいえ' case"""
    
    with driver.session() as session:
        # Find the specific word with "I ie" romaji
        result = session.run("""
            MATCH (w:Word)
            WHERE w.romaji = 'I ie' AND w.kanji = 'いいえ'
            RETURN id(w) as word_id, w.lemma, w.kanji, w.hiragana, w.romaji, w.vdrj_standard_reading
        """)
        
        logger.info("Fixing specific 'いいえ' case:")
        logger.info("=" * 50)
...
def main():
    ...
    try:
        ...
        fixed_count = fix_iie_specific(driver)
