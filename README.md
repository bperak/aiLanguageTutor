...
- **Code persistence**: In `docker-compose.yml` the backend and frontend source folders are bind-mounted, so edit on the host to keep changes. In `docker-compose.server.yml` the code is baked into the image; rebuild (`docker compose -f docker-compose.server.yml up -d --build`) to deploy code updates.
...
- `backend/migrations/init.sql` is mounted into Postgres as `docker-entrypoint-initdb.d/init.sql` and runs only on the **first** initialization of a fresh DB volume.
- Production uses a persistent Postgres volume, so schema updates must be applied after deploys.
- On backend startup, the API applies curated SQL migrations from `backend/migrations/versions/*.sql` exactly once (tracked in a `schema_migrations` table).
...
### 6. Cloudflare Tunnel Setup (Remote Access)

If you need to access the app from outside your network (e.g., school/corporate networks that block direct IP access), use Cloudflare Tunnel:
...
#### Create Tunnels
```bash
# Create tunnel for frontend (port 3000)
cloudflared tunnel create ailanguagetutor-frontend

# Create tunnel for backend API (port 8000)  
cloudflared tunnel create ailanguagetutor-api
```

#### Configure DNS Routes
In Cloudflare dashboard, add CNAME records:
- `ailanguagetutor.yourdomain.com` → frontend tunnel
- `api.ailanguagetutor.yourdomain.com` → backend tunnel

#### Run Tunnels
```bash
# Terminal 1: Frontend tunnel
cloudflared tunnel run --url http://localhost:3000 ailanguagetutor-frontend

# Terminal 2: Backend API tunnel
cloudflared tunnel run --url http://localhost:8000 ailanguagetutor-api
```

#### Update Environment for Cloudflare
Add to your `.env` file:
```
NEXT_PUBLIC_API_BASE_URL=https://api.ailanguagetutor.yourdomain.com
```
...
- **Frontend**: https://ailanguagetutor.syntagent.com
- **Backend API**: https://api.ailanguagetutor.syntagent.com
...
### Script Learning (Japanese Kana)
- `GET /api/v1/script/items` - List script items (Hiragana/Katakana) with filtering
- `GET /api/v1/script/items/{item_id}` - Get a specific script item by ID
- `POST /api/v1/script/practice/check` - Check a practice answer and record attempt
- `GET /api/v1/script/progress/summary` - Get overall progress summary
- `GET /api/v1/script/progress/items` - Get per-item progress statistics

**Script Learning Feature**: Learn Japanese scripts (Hiragana and Katakana) through interactive practice modes:
- **Kana → Romaji**: Type romaji when shown kana
- **Romaji → Kana**: Type kana when shown romaji  
- **Multiple Choice**: Select correct answer from options

Progress is tracked per item with mastery levels and correct rates. All practice attempts are recorded in the `conversation_interactions` table for analytics.
