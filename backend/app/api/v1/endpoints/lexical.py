...
async def get_node_details(
    ...
    result = await session.run(query, kanji=center_node["kanji"])
    rec = await result.single()
    ...
    """
    count_result = await session.run(count_query, kanji=center_node["kanji"])
...
async def get_lexical_graph(
    center: str,
    searchField: str = Query("kanji", alias="searchField"),
    depth: int = 1,
    session: AsyncSession = Depends(get_neo4j_session),
) -> Dict[str, Any]:
    """
