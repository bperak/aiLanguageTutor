...
async def get_profile_status(
    ...
    has_profile_data = profile_result.scalar_one_or_none() is not None
    
    return ProfileStatusResponse(
        profile_completed=status_data["completed"],
        profile_skipped=status_data["skipped"],
        profile_completed_at=current_user.profile_completed_at if status_data["completed"] else None,
...
async def complete_profile(
    ...
    try:
        ...
        # Generate learning path in background (don't wait)
        try:
            from app.db import get_neo4j_session
            async for neo4j_session in get_neo4j_session():
                ...
                await learning_path_service.save_learning_path(
                    db=db,
                    user_id=current_user.id,
                    path_data=path_data
                )
        ...
        return {"status": "completed", "message": "Profile completed successfully"}
