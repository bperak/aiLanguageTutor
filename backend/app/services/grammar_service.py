...
def _prettify_pattern_romaji(pattern: Dict[str, Any]) -> Dict[str, Any]:
    """
    Apply romaji prettification to pattern dictionary fields.
    
    Modifies in place and returns the pattern for chaining.
    """
    romaji_fields = ["pattern_romaji", "textbook_form_romaji", "example_romaji"]
    for field in romaji_fields:
        if field in pattern and pattern[field]:
            pattern[field] = prettify_romaji_template(pattern[field])
    return pattern
    ...
    async def get_patterns(
        ...
        try:
            ...
            async for record in result:
                pattern = dict(record)
                _prettify_pattern_romaji(pattern)
