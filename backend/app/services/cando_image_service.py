...
def extract_image_specs(lesson_data: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any], List[str]]]:
    ...
    def is_valid_image_path(path: str) -> bool:
        ...
        # In Docker, we generate into /app/images (mounted from repo ./images).
        # Paths are stored like "images/lessons/cando/<id>/<file>.png".
        candidates = [
            Path("/app") / path,             # /app/images/... in backend container
            _repo_root() / path,             # best-effort for non-docker layouts
        ]
...
def ensure_image_paths_for_lesson(
    ...
    *,
    ...
    Images are generated to repo ./images/lessons/cando/{can_do_id}/ (mounted in Docker as /app/images),
    and the frontend mounts ./images -> /app/public/images so Next.js serves them at /images/*.
    """
    ...
    if output_base_dir is None:
        # Default: write to /app/images (Docker volume mount) if present.
        base_images = Path("/app/images")
        if not base_images.exists():
            # Fallback to repo-local ./images for non-docker execution.
            base_images = _repo_root() / "images"
        image_dir = base_images / "lessons" / "cando" / sanitized_id
    else:
        # Allow override for testing or special cases (expects base "images" dir)
