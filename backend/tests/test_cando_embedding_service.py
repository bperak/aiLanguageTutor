...
async def test_update_cando_embedding_success(cando_embedding_service, mock_neo4j_session):
    ...
    # Title generation is an optional side-effect; this test focuses on embeddings only.
    cando_embedding_service.title_service = None
...
async def test_find_similar_candos(cando_embedding_service, mock_neo4j_session):
    ...
    mock_record1.get.side_effect = lambda key, default=None: {
        "uid": "cando_2",
        "level": "A1",
        "topic": "Greetings",
    ...
    mock_result = AsyncMock()
    # AsyncMock supports async iteration via __aiter__.return_value
    mock_result.__aiter__.return_value = [mock_record1, mock_record2]
...
async def test_find_similar_candos_fallback(cando_embedding_service, mock_neo4j_session):
    ...
    mock_target_record.get.side_effect = lambda key, default=None: {
        "uid": "cando_2",
        "level": "A1",
        "topic": "Greetings",
        "skillDomain": "Interaction",
    ...
    mock_fallback_result.__aiter__.return_value = [mock_target_record]
...
async def test_batch_generate_cando_embeddings(cando_embedding_service, mock_neo4j_session):
    ...
    # batch_generate_cando_embeddings() also calls neo4j_session.run() for UPDATEs.
    # Provide a side_effect that returns batches for the batch query and a no-op AsyncMock for updates.
    state = {"batch_calls": 0}

    async def run_side_effect(query, params=None):  # type: ignore[no-untyped-def]
        q = str(query)
        p = params or {}

        # Detect the batch fetch query by the presence of SKIP/LIMIT and returning uid/description fields.
        if "RETURN c.uid AS uid" in q and "SKIP $skip" in q and "LIMIT $limit" in q:
            state["batch_calls"] += 1
            return mock_result if state["batch_calls"] == 1 else empty_result

        # For updates, return a harmless result object.
        return AsyncMock()
