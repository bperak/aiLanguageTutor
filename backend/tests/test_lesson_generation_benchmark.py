...
# These tests can call real AI provider APIs and may be slow/costly.
# They are opt-in to avoid unexpected usage/costs in normal CI/dev runs.
pytestmark = [pytest.mark.slow, pytest.mark.integration]


def _benchmarks_enabled() -> bool:
    """
    Check whether AI benchmark tests are explicitly enabled.

    Returns:
        bool: True if enabled via env var.
    """
    return os.getenv("RUN_AI_BENCHMARKS", "0") == "1"
...
async def test_benchmark_lesson_generation():
    ...
    if not _benchmarks_enabled():
        pytest.skip("Set RUN_AI_BENCHMARKS=1 to run AI benchmark tests.")
