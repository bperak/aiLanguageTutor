...
RUN_TARGET_BACKEND = os.environ.get("TARGET_BACKEND")
...
def test_start_and_dump(can_do_id: str) -> None:
    ...
    """
    if not RUN_TARGET_BACKEND:
        pytest.skip("Set TARGET_BACKEND to run output-capture tests against a running backend.")
